...
const KassaDet = () => {
    ...
    const fetchCashboxDetails = async (idToFetch) => {
        ...
        setError(null);
        try {
            const response = await fetch(`https://app.nurcrm.kg/api/construction/cashboxes/${idToFetch}/detail/owner/`, {
                headers: {
            ...
            console.error(`Failed to fetch cashbox details for ID ${idToFetch}:`, err);
            setError(`Не удалось загрузить данные кассы: ${err.message}. Пожалуйста, попробуйте еще раз.`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAddCashbox = async () => {
        try {
            ...
            setError("Не удалось добавить операцию по кассе. Пожалуйста, проверьте данные и попробуйте еще раз.");
        }
    };
    ...
    const handleEditCashbox = async () => {
        ...
        try {
            const response = await fetch(`https://app.nurcrm.kg/api/construction/cashboxes/${cashboxId}/`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: "Bearer " + localStorage.getItem('accessToken')
    ...
    const handleDeleteCashbox = async () => {
        ...
        try {
            const response = await fetch(`https://app.nurcrm.kg/api/construction/cashflows/${cashboxId}/`, {
                method: 'DELETE',
                headers: {
                    Authorization: "Bearer " + localStorage.getItem('accessToken')
                }
    ...
    return (
        <div className="vitrina">
            <div className="vitrina__header">
                <div className="vitrina__tabs">
                    <span
                        className={`vitrina__tab ${activeFlowType === 'expense' ? 'vitrina__tab--active' : ''}`}
                        onClick={() => setActiveFlowType('expense')}
                    >
                        Расход
                    </span>
                    <span
                        className={`vitrina__tab ${activeFlowType === 'income' ? 'vitrina__tab--active' : ''}`}
                        onClick={() => setActiveFlowType('income')}
                    >
                        Приход
                    </span>
                    <span
                        className={`vitrina__tab ${activeFlowType === 'all' ? 'vitrina__tab--active' : ''}`}
                ...
                <br/>
                <button className=" sklad__add vitrina__add-expense-button vitrina__button vitrina__button--delete " onClick={() => setShowAddCashboxModal(true)}>
                    Добавить операцию
                </button>
            </div>
      ...
      <div className="table-wrapper"> 
                    ...
                    <p>
                        ...
                        {activeFlowType === 'income' ? ' (Приходы).' :
                         activeFlowType === 'expense' ? ' (Расходы).' :
                         '.'
                        }
                    </p>
                )}
                        ...
                        <div className="vitrina__filter-content">
                            ...
                            <div className="vitrina__filter-section">
                                <div className="vitrina__search-wrapper">
                                    ...
                                    <input type="text" placeholder="Поиск потоков" className="vitrina__search" />
                                </div>
                            </div>
                            <div className="vitrina__filter-section">
                    ...
                    <div className="vitrina__modal vitrina__modal--add">
                        ...
                        <div className="vitrina__modal-section">
                            ...
                            <input
                                ...
                                onChange={(e) => setNewCashbox({ ...newCashbox, amount: parseFloat(e.target.value) || 0 })}
                            />
                        </div>
                        <div className="vitrina__modal-section">
                    ...
                    <div className="vitrina__modal vitrina__modal--edit">
                        ...
                        <div className="vitrina__modal-section">
                            ...
                            <input
                                ...
                                onChange={(e) => setSelectedCashbox({ ...selectedCashbox, title: e.target.value })}
                                className="vitrina__modal-input"
                            />
                        </div>
                        <div className="vitrina__modal-footer">
                            {/* <button className="vitrina__button vitrina__button--reset" onClick={handleDeleteCashbox}>Удалить кассу</button> */}
                            <button className="vitrina__button vitrina__button--save" onClick={handleEditCashbox}>Сохранить изменения</button>
                        </div>
                    </div>
                </div>
            )}
