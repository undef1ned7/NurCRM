...
export default function SchoolInvoices() {
  ...
  const byGroup = useMemo(() => {
    ...
    filteredRows.forEach((r) => {
      ...
      if (!map.has(k)) map.set(k, { name: r.groupName, amount: 0, count: 0, courseName: r.courseName });
      const v = map.get(k);
      v.amount += Number(r.amount || 0);
      v.count  += 1;
    });
  ...
  return (
    <div className={s.inv}>
      {/* Header */}
      <div className={s.inv__header}>
        <div>
          <h2 className={s.inv__title}>Счета · Аналитика</h2>
          <p className={s.inv__subtitle}>
            Период: <b>{periodLabel}</b>
          </p>
        </div>

        {/* Toolbar */}
        <div className={s.inv__toolbar}>
          <div className={s.inv__filters}>
            <select
              className={s.inv__input}
              value={year}
          ...
          <div className={s.inv__right}>
            <div className={s.inv__search}>
              <FaSearch className={s["inv__search-icon"]} aria-hidden />
              <input
                className={s["inv__search-input"]}
            ...
            <button
              className={`${s.inv__btn} ${s["inv__btn--secondary"]}`}
              onClick={loadAll}
              title="Обновить"
              aria-label="Обновить"
            >
              <FaSync /> <span className={s.inv__btnText}>Обновить</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {/* States */}
      {loading && <div className={s.inv__alert}>Загрузка…</div>}
      {!!error && !loading && <div className={s.inv__alert}>{error}</div>}

      {/* Content */}
      {!loading && !error && (
        <>
          <div className={s.inv__kpis}>
            <div className={s.inv__kpiCard}>
              <div className={s.inv__kpiIcon}>₮</div>
              <div className={s.inv__kpiBody}>
                <div className={s.inv__kpiLabel}>Начислено за период</div>
                <div className={s.inv__kpiValue}>{money(totals.amount)}</div>
              </div>
            </div>
            ...
            <div className={s.inv__kpiCard}>
              <div className={s.inv__kpiIcon}>👥</div>
              <div className={s.inv__kpiBody}>
                <div className={s.inv__kpiLabel}>Активных учащихся</div>
                <div className={s.inv__kpiValue}>{totals.students}</div>
              </div>
            </div>

            <div className={s.inv__kpiCard}>
              <div className={s.inv__kpiIcon}>Ø</div>
              <div className={s.inv__kpiBody}>
                <div className={s.inv__kpiLabel}>Средний чек</div>
                <div className={s.inv__kpiValue}>{money(totals.avg)}</div>
              </div>
            </div>
          </div>

          <h3 className={s.inv__sectionTitle}>Курсы · сумма и ученики</h3>
          <div className={s.inv__list}>
            {byCourse.map((c, idx) => (
              <div key={`${c.name}-${idx}`} className={s.inv__card}>
                <div className={s["inv__card-left"]}>
                  <div className={s.inv__avatar} aria-hidden>
                    {(c.name || "•").charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <div className={s["inv__name-row"]}>
                      <p className={s.inv__name}>{c.name || "—"}</p>
                      <span className={s.inv__chip}>{c.count} уч.</span>
                    </div>
                    <div className={s.inv__meta}>
                      <span>Сумма: <b>{money(c.amount)}</b></span>
                    </div>
                    <div className={s.inv__bar}>
                      <div
                        className={s.inv__barFill}
            ...
            {byCourse.length === 0 && (
              <div className={s.inv__empty}>Нет данных по курсам за выбранный период.</div>
            )}
          </div>

          <h3 className={s.inv__sectionTitle}>Группы · сумма и ученики</h3>
          <div className={s.inv__list}>
            {byGroup.map((g, idx) => (
              <div key={`${g.name}-${idx}`} className={s.inv__card}>
                <div className={s["inv__card-left"]}>
                  <div className={s.inv__avatar} aria-hidden>
                  ...
                  <div>
                    <div className={s["inv__name-row"]}>
                      <p className={s.inv__name}>
                        {g.name || "—"} <span className={s.inv__muted}>· {g.courseName || "—"}</span>
                      </p>
                      <span className={s.inv__chip}>{g.count} уч.</span>
                    </div>
                    <div className={s.inv__meta}>
            ...
            {byGroup.length === 0 && (
              <div className={s.inv__empty}>Нет данных по группам за выбранный период.</div>
            )}
          </div>
        </>
      )}
