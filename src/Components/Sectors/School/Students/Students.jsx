...
const ENDPOINT_COURSES  = "/education/courses/";
const ENDPOINT_GROUPS   = "/education/groups/";
const STUDENT_ATT       = (id) => `/education/students/${id}/attendance/`; // ⟵ добавили
...
const SchoolStudents = () => {
  ...
  /* ====== ПОСЕЩАЕМОСТЬ В КАРТОЧКЕ СТУДЕНТА ====== */
  // id -> { items:[], next:null, loading:false, error:"" }
            ...
            <div key={st.id} className="Schoolstudents__card">
              <div className="Schoolstudents__left">
                ...
                <div className="Schoolstudents__info">
                  ...
                  {/* ==== Посещаемость выбранного студента ==== */}
                  <details
                    className="Schoolstudents__att"
                    onToggle={(e)=>onToggleStudentAtt(st.id, e.currentTarget.open)}
                  >
                    ...
                    {att[st.id]?.next && !att[st.id]?.loading && (
                      <button
                        ...
                        onClick={()=>loadStudentAttendance(st.id, att[st.id].next)}
                      >
                        Показать ещё
                      </button>
                    )}
